...
const Transactions = () => {
  const { currentAccount, Transcation } = useContext(TransactionContext);
  return (
    <div className=" flex w-full justify-center items-center 2xl:px-20 gradient-bg-transactions">
      <div className="flex flex-col md:p-12 py-12 px-4">
        ...
        <div className="flex flex-wrap justify-center items-center mt-10">
          {Transcation.reverse().map((transaction, i) => (
            <Transactioncard key={i} {...transaction} />
          ))}
        </div>
        <p className="text-white text-base text-center my-10">
          Note: if Transaction is not appear then reload the page
        </p>
      </div>
    </div>
  );
